...
def main(config, parser):
    ...
    video_model = slowfast_detection(config).eval().to(device)
        ...
        if len(cap.stack) == 25:
            ...
            if yolo_preds.pred[0].shape[0]:
                ...
                for tid, slowfaster_pred in zip(yolo_preds.pred[0][:, 5].tolist(), np.array(slowfaster_preds)):
                    ...
                    for index in temp:
                        labels.append([ava_labelnames[index+1], slowfaster_pred[index]//0.01/100])
                    print(tid, labels)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cfg", dest="cfg_files", help="Path to the config files",
                        default=["D:\git\YoloSlowfastDangerousDetector\Slowfast\demo\AVA\SLOWFAST_32x2_R101_50_50_mini.yaml"], nargs="+")
